...
premiumPlan::premiumPlan(int userID):currentUserID(userID){}
...
bool premiumPlan::activatePremiumPlan(){
    if(!isPaid){
        throw UserException("Payment process is yet to be done to unlock premium access!");
    }
    isActive=true;
...
bool premiumPlan::deactivatePremiumPlan(){
    if(!isActive){
        throw UserException("The premium plan is in inactive state already!");
    }
    isActive=false;
...
double premiumPlan::applyPromoCodeDeal(){
    ...
    if(guess < 1 || guess > 100)
        throw UserException("Invalid guess! Number must be between 1 and 100.");
...
double premiumPlan::finalPricing(){
    if(packagePrice <= 0)
        throw UserException("Invalid package price! Price must be greater than 0.");
...
bool premiumPlan::hasDonePaymentProcess(){
    ...
    if(!updatePremiumStatusInFile("users.txt"))
        throw FileException();
...
void premiumPlan::dietChart(){
    ...
    if(w <= 0)
        throw UserException("Invalid weight! Weight must be greater than 0.");
    ...
    if(h <= 0)
        throw UserException("Invalid height! Height must be greater than 0.");
    ...
    if(bmi<18.5){
        ...
        for(int i=0;i<(int)menuPerMeal.size();i++){
            cout<<mealTimes[i]<<":"<<endl;
            for(int j=0;j<(int)menuPerMeal[i].size();j++){
...
double premiumPlan::daily_calorie_intake_tracker(){
    ...
    if(items < 0)
        throw UserException("Invalid input! Number of food items cannot be negative.");
    ...
    for (int i=1; i<=items;i++) {
        ...
        if(servings <= 0)
            throw UserException("Invalid servings! Servings must be greater than 0.");
...
double premiumPlan::daily_calorie_burner_tracker() {
    ...
    if(n < 0)
        throw UserException("Invalid input! Number of activities cannot be negative.");
    ...
    for(int i = 0; i < n; i++) {
        ...
        if(minutes <= 0)
            throw UserException("Invalid duration! Duration must be greater than 0.");
...
void premiumPlan::net_calorie_per_day_tracker() {
    if(lastIntake == 0.0 && lastBurned == 0.0)
        throw UserException("No calorie data found! Please use the Calorie Intake and Burn Trackers first.");
...
void premiumPlan::sigma_power_index(){
    if(lastIntake == 0.0 && lastBurned == 0.0 && weight == 0.0)
        throw UserException("Insufficient data for Sigma Power Index! Please complete the calorie trackers and set your weight first.");
    ...
    if (burned >= 800)      trainingScore = 35;
    else if (burned >= 600) trainingScore = 30;
    else if (burned >= 400) trainingScore = 24;
    else if (burned >= 200) trainingScore = 15;
    else                    trainingScore = 6;

    if (fabs(netCalories) <= 150)      nutritionScore = 30;
    else if (fabs(netCalories) <= 300) nutritionScore = 24;
    else if (fabs(netCalories) <= 500) nutritionScore = 18;
    else if (fabs(netCalories) <= 800) nutritionScore = 10;
    else                               nutritionScore = 4;

    if (burned > 0 && intake > 0) consistencyScore = 20;
    else if (burned > 0)          consistencyScore = 12;
    else                          consistencyScore = 5;

    if (weight > 0) awarenessScore = 15;
    else            awarenessScore = 6;

    int sigmaIndex = trainingScore + nutritionScore + consistencyScore + awarenessScore;
...
bool premiumPlan::loadPremiumStatusFromUsersFile(const std::string& filePath) {
    ...
    while (file >> id >> username >> password >> premium_flag) {
        if (id == currentUserID) {
            found = true;
            if (premium_flag == 1) {
                isPaid = true;
                isActive = true;
                std::cout << "Premium feature unlocked for user ID " << currentUserID << ".\n";
                return true;
            } else {
                isPaid = false;
                isActive = false;
                std::cout << "Premium feature is not enabled for user ID " << currentUserID << ".\n";
                return false;
            }
        }
    }
    ...
    if (!found)
        throw UserException("User ID " + to_string(currentUserID) + " not found in file!");

    return false;
}

bool premiumPlan::checkPremiumAccess(const std::string& filePath) {
    std::ifstream file(filePath);
    if (!file.is_open())
        throw FileException();
    ...
    while (file >> id >> username >> password >> premium_flag) {
        if (id == currentUserID && premium_flag == 1) {
            return true;
        }
    }
...
bool premiumPlan::updatePremiumStatusInFile(const std::string& filePath)
{
    ...
    while (inFile >> id >> username >> password >> premium_flag)
    {
        if (id == currentUserID)
            premium_flag = 1;

        users.push_back({id, username, password, premium_flag});
    }
    ...
    cout << "Premium status updated successfully in file.\n";
    return true;
}
